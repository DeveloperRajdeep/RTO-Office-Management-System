...
<div id="apDiv1">
  <div id="apDiv6">
    <div id="apDiv8">
      <table width="834" height="126" border="1">
        <tr>
          <td width="65">Tax ID</td>
          <td width="82">Vehicle Number</td>
          <td width="82">Vehicle Price</td>
          <td width="93">Tax Paid For</td>
          <td width="84">Tax Pay</td>
          <td width="83">Date</td>
          <td width="99">Approve Date</td>
          <td width="68">Status</td>
          <td width="120">View LLR ID CARD</td>
        </tr>
         <?php
  $sql="select DISTINCT(usr_id),vnumber,myprice,myqty,mytotal,date,approvedate,status from tax where usr_id=$user_id";
  $res=mysql_query($sql);
  while($row=mysql_fetch_array($res))
  {
  ?>
        <tr>
          <td>TAX00<?php echo $row['usr_id']?></td>
         ...
         <td><?php echo $row['myqty'] ?>years</td>
          ...
          <td><a href="print_tax.php" target="_blank"><img src="printer.png" /></a></td>
        </tr>
		<?php
      }
	  ?>
    ...
    <div id="apDiv7">Status of Tax PAY</div>
  </div>
